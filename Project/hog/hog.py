...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            points = take_turn(num_rolls, score0, score1, dice, goal)
            score0 += points
            addition = hog_pile(score0, score1)
            score0 += addition

        elif who == 1:
            num_rolls = strategy1(score1, score0)
            points = take_turn(num_rolls, score1, score0, dice, goal)
            score1 += points
            addition = hog_pile(score1, score0)
            score1 += addition
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    max_score, current_roll = 0, 1
    while current_roll <= 10:
        averaged_dice = make_averaged(roll_dice, total_samples)
        current_roll_score = averaged_dice(current_roll, dice)
        if current_roll_score > max_score:
            max_score = current_roll_score
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    total_score = score + hefty_hogs(score, opponent_score)
    if hog_pile(total_score, opponent_score) > 0:
        return 0
