...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    topic = [remove_punctuation(lower(word)) for word in topic]
...
def accuracy(typed, reference):
    ...
    for i in range(len(reference_words)):
        try:
            ...
            accuracy.append(0)
    try:
        return round(sum(accuracy) / len(typed_words), 2)
    except:
        return 0.0
    # END PROBLEM 3
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    numerator = len(typed) / 5
    denominator = elapsed / 60
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    return typed_word if min(difference.values()) > limit else min(difference, key=difference.get)
    # END PROBLEM 5
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == goal:
        ...
        substitute = flag + minimum_mewtations(start[1:], goal[1:], limit - flag)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    d = {'id' : user_id, 'progress': progress / len(prompt)}
...
def time_per_word(words, times_per_player):
    ...
    """
    # BEGIN PROBLEM 9
    timestamps = [[times_per_player[j][i+1] - times_per_player[j][i] for i in range(len(times_per_player[j])-1)] for j in range(len(times_per_player))]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    fastest_list = [[] for p in player_indices]

    for word_index in word_indices:
        fastest_time = float('inf')
        player = 0
        for player_num in player_indices:
            if time(match, player_num, word_index) < fastest_time:
                fastest_time = time(match, player_num, word_index)
                player = player_num
        fastest_list[player].append(word_at(match, word_index))
    return fastest_list
    # END PROBLEM 10
